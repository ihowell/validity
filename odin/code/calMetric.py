...
def tpr95(name):
    ...
    # calculate baseline
    T = 1
    cifar = np.loadtxt('./softmax_scores/confidence_Base_In.txt',
                       delimiter=',')
    other = np.loadtxt('./softmax_scores/confidence_Base_Out.txt',
                       delimiter=',')
    if name == "CIFAR-10":
        start = 0.1
        end = 1
    ...
    other = np.loadtxt('./softmax_scores/confidence_Our_Out.txt',
                       delimiter=',')
    ...
    labels = np.concatenate([np.zeros(X1.shape[0]), np.ones(Y1.shape[0])])
    fpr, tpr, _ = roc_curve(labels, np.concatenate([X1, Y1]))
...
def metric(nn, data):
    if nn == "densenet10" or nn == "wideresnet10" or nn == "resnet34":
        indis = "CIFAR-10"
    ...
    print("{:20}{:13.1f}%{:>18.1f}% ".format("FPR at TPR 95%:", fprBase * 100,
                                             fprNew * 100))
    print("{:20}{:13.1f}%{:>18.1f}%".format("Detection error:",
                                            errorBase * 100, errorNew * 100))
    print("{:20}{:13.1f}%{:>18.1f}%".format("AUROC:", aurocBase * 100,
                                            aurocNew * 100))
    print("{:20}{:13.1f}%{:>18.1f}%".format("AUPR In:", auprinBase * 100,
                                            auprinNew * 100))
    print("{:20}{:13.1f}%{:>18.1f}%".format("AUPR Out:", auproutBase * 100,
                                            auproutNew * 100))
