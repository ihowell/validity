...
def get_Mahalanobis_score(model, test_loader, num_classes, outf, out_flag,
                          net_type, sample_mean, precision, layer_index,
                          magnitude):
    '''
    ...
    if out_flag == True:
        ...
        temp_file_name = '%s/confidence_Ga%s_Out.txt' % (outf,
                                                         str(layer_index))
    ...
    for data, target in test_loader:
        ...
        for i in range(num_classes):
            ...
            term_gau = -0.5 * torch.mm(
                torch.mm(zero_f, precision[layer_index]), zero_f.t()).diag()
            if i == 0:
        ...
        pure_gau = -0.5 * torch.mm(
            torch.mm(zero_f, Variable(precision[layer_index])),
            zero_f.t()).diag()
...
def get_posterior(model, net_type, test_loader, magnitude, temperature, outf,
                  out_flag):
    '''
...
def get_Mahalanobis_score_adv(model, test_data, test_label, num_classes, outf,
                              net_type, sample_mean, precision, layer_index,
                              magnitude):
    '''
...
def get_LID(model, test_clean_data, test_adv_data, test_noisy_data, test_label,
            num_output):
    '''
    ...
    for data_index in range(int(np.floor(test_clean_data.size(0) /
                                         batch_size))):
        ...
        for overlap in overlap_list:
            ...
            for j in range(num_output):
                ...
                lid_noisy_score = mle_batch(X_act[j],
                                            X_act_noisy[j],
                                            k=overlap)
            ...
            for i in range(1, num_output):
                ...
                LID_adv_concat = np.concatenate(
                    (LID_adv_concat, LID_adv_list[i]), axis=1)
                LID_noisy_concat = np.concatenate(
                    (LID_noisy_concat, LID_noisy_list[i]), axis=1)
