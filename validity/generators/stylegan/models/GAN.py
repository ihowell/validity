...
from models.Blocks import (DiscriminatorBlock, DiscriminatorTop, GSynthesisBlock, InputBlock)
from models.CustomLayers import (EqualizedConv2d, EqualizedLinear, PixelNormLayer, Truncation)
...
class GMapping(nn.Module):
    ...
    def __init__(self,
        ...
        layers.append(('dense0',
                       EqualizedLinear(self.latent_size,
                                       self.mapping_fmaps,
                                       gain=gain,
                                       lrmul=mapping_lrmul,
        ...
        for layer_idx in range(1, mapping_layers):
            ...
            layers.append(('dense{:d}'.format(layer_idx),
                           EqualizedLinear(fmaps_in,
                                           fmaps_out,
                                           gain=gain,
                                           lrmul=mapping_lrmul,
                                           use_wscale=use_wscale)))
    ...
    def __init__(self,
        ...
        for res in range(3, resolution_log2 + 1):
            ...
            rgb_converters.append(
                EqualizedConv2d(channels, num_channels, 1, gain=1, use_wscale=use_wscale))
    ...
    def forward(self, dlatents_in, depth=0, alpha=0., labels_in=None):
        ...
        if self.structure == 'fixed':
            ...
            if depth > 0:
                ...
                straight = self.to_rgb[depth](self.blocks[depth - 1](
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.g_mapping = GMapping(latent_size,
                                  dlatent_size,
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None):
        ...
        if not self.conditional:
            if labels_in is not None:
                warnings.warn("Generator is unconditional, labels_in will be ignored")
        else:
        ...
        if self.training:
            ...
            if self.style_mixing_prob is not None and self.style_mixing_prob > 0:
                ...
                layer_idx = torch.from_numpy(
                    np.arange(self.num_layers)[np.newaxis, :,
                                               np.newaxis]).to(latents_in.device)
                cur_layers = 2 * (depth + 1)
                mixing_cutoff = random.randint(
                    1, cur_layers) if random.random() < self.style_mixing_prob else cur_layers
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        for res in range(resolution_log2, 2, -1):
            ...
            if conditional:
                r = 2**(res)
                self.embeddings.append(Embedding(n_classes, (num_channels // 2) * r * r))
    ...
    def forward(self, images_in, depth, alpha=1., labels_in=None):
        ...
        if self.structure == 'fixed':
            if self.conditional:
                ...
                embedding_in = embedding_in.view(images_in.shape[0], -1, images_in.shape[2],
                                                 images_in.shape[3])
            ...
            if depth > 0:
                if self.conditional:
                    embedding_in = self.embeddings[self.depth - depth - 1](labels_in)
                    embedding_in = embedding_in.view(images_in.shape[0], -1,
                                                     images_in.shape[2], images_in.shape[3])
                    images_in = torch.cat([images_in, embedding_in], dim=1)

                residual = self.from_rgb[self.depth - depth](
...
class StyleGAN:
    ...
    def train(self,
        ...
        fixed_labels = None
        if self.conditional:
            fixed_labels = torch.linspace(0, self.n_classes - 1,
                                          num_samples).to(torch.int64).to(self.device)
        ...
        for current_depth in range(start_depth, self.depth):
            ...
            for epoch in range(1, epochs[current_depth] + 1):
                ...
                for i, batch in enumerate(data, 1):
                    ...
                    dis_loss = self.optimize_discriminator(gan_input, images, current_depth,
                                                           alpha, labels)
                    ...
                    if i % int(total_batches / feedback_factor + 1) == 0 or i == 1:
                        ...
                        gen_img_file = os.path.join(
                            output, 'samples', "gen_" + str(current_depth) + "_" + str(epoch) +
                            "_" + str(i) + ".png")
                ...
                if epoch % checkpoint_factor == 0 or epoch == 1 or epoch == epochs[
                    ...
                    gen_optim_save_file = os.path.join(
                        save_dir,
                        "GAN_GEN_OPTIM_" + str(current_depth) + "_" + str(epoch) + ".pth")
                    dis_optim_save_file = os.path.join(
                        save_dir,
                        "GAN_DIS_OPTIM_" + str(current_depth) + "_" + str(epoch) + ".pth")
                    ...
                    if self.use_ema:
                        gen_shadow_save_file = os.path.join(
                            save_dir,
                            "GAN_GEN_SHADOW_" + str(current_depth) + "_" + str(epoch) + ".pth")
