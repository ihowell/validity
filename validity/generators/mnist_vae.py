...
class MnistVAE(nn.Module):
    def __init__(self, beta=1.):
        ...
        self.dec_layers = nn.ModuleList([
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(128, 64, 5, stride=2, output_padding=1, padding=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        kl = kl.mean()
        posterior_nll = posterior_nll.mean()
...
def train(beta=1., max_epochs=1000, batch_size=64, data_root='./datasets/', cuda_idx=0):
    ...
    writer = SummaryWriter(f'vae/mnist_{beta}')
    ...
    for epoch in range(max_epochs):
        ...
        for data, _ in tqdm(train_loader):
            ...
            writer.add_scalar('train/nelbo', nelbo, step)
            writer.add_scalar('train/kl', kl, step)
            writer.add_scalar('train/posterior_nll', posterior_nll, step)
        ...
        for data, _ in tqdm(val_loader):
            ...
            with torch.no_grad():
                data = data.cuda()
                nelbo, kl, posterior_nll, x_hat = vae(data)
                writer.add_scalar('val/nelbo', nelbo, step)
                writer.add_scalar('val/kl', kl, step)
                writer.add_scalar('val/posterior_nll', posterior_nll, step)
                writer.add_images('val/reconstruction',
                                  torch.cat([data, x_hat], dim=2)[:3], step)
                step += 1

                val_loss.append(nelbo)
