...
from validity.fid.fid_score import compute_statistics_of_generator, load_statistics, calculate_frechet_distance
from validity.fid.inception import InceptionV3
...
def create_generator_vae(model, batch_size, num_total_samples):
    ...
    for i in range(num_iters):
        with torch.no_grad():
            ...
            output_img = output.mean if isinstance(
                output,
                torch.distributions.bernoulli.Bernoulli) else output.mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',
                        ...
                        default='mnist',
                        choices=[
                            'cifar10', 'mnist', 'omniglot', 'celeba_64',
                            'celeba_256', 'imagenet_32', 'ffhq',
                            'lsun_bedroom_128', 'stacked_mnist',
    ...
    # KL annealing
    parser.add_argument('--kl_anneal_portion',
                        type=float,
                        default=0.3,
    ...
    parser.add_argument('--kl_const_coeff',
                        type=float,
                        default=0.0001,
    ...
    # latent variables
    parser.add_argument('--num_latent_scales',
                        type=int,
                        default=1,
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'Settings this to true will set different number of groups per scale.')
    parser.add_argument('--min_groups_per_scale',
                        type=int,
                        default=1,
                        help='the minimum number of groups per scale.')
    # encoder parameters
    parser.add_argument('--num_channels_enc',
                        type=int,
                        default=32,
                        help='number of channels in encoder')
    parser.add_argument('--num_preprocess_blocks',
                        type=int,
                        default=2,
                        help='number of preprocessing blocks')
    parser.add_argument('--num_preprocess_cells',
                        type=int,
                        default=3,
    ...
    # decoder parameters
    parser.add_argument('--num_channels_dec',
                        type=int,
                        default=32,
                        help='number of channels in decoder')
    parser.add_argument('--num_postprocess_blocks',
                        type=int,
                        default=2,
                        help='number of postprocessing blocks')
    parser.add_argument('--num_postprocess_cells',
                        type=int,
                        default=3,
                        help='number of cells per block')
    ...
    parser.add_argument(
        ...
        default=10,
        help=
        'number of mixture components in decoder. set to 1 for Normal decoder.'
    )
    # NAS
