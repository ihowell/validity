...
from validity.generators.nvae.utils import one_hot
...
class DiscLogistic:
    ...
    def log_prob(self, samples):
        ...
        log_prob_mid_safe = torch.where(cdf_delta > 1e-5, torch.log(torch.clamp(cdf_delta, min=1e-10)),
        ...
        log_probs = torch.where(samples < -0.999, log_cdf_plus,
                                torch.where(samples > 0.99, log_one_minus_cdf_min, log_prob_mid_safe))  # B, 3, H, W
...
class DiscMixLogistic:
    ...
    def log_prob(self, samples):
        ...
        # which is mapped to 0.9922
        log_probs = torch.where(samples < -0.999, log_cdf_plus,
                                torch.where(samples > 0.99, log_one_minus_cdf_min, log_prob_mid_safe))  # B, 3, M, H, W
