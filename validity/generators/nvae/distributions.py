...
class Normal:
    def __init__(self, mu, log_sigma, temp=1.):
        ...
        self.sigma = torch.exp(
            log_sigma) + 1e-2  # we don't need this after soft clamp
        if temp != 1.:
            self.sigma *= temp
...
class DiscLogistic:
    def __init__(self, param):
        ...
        self.log_scales = torch.clamp(param[:, self.num_c:, :, :],
                                      min=-8.0)  # B, 3, H, W
    ...
    def log_prob(self, samples):
        ...
        log_prob_mid_safe = torch.where(
            cdf_delta > 1e-5, torch.log(torch.clamp(cdf_delta, min=1e-10)),
    ...
    def sample(self):
        u = torch.Tensor(self.means.size()).uniform_(1e-5, 1. -
                                                     1e-5).cuda()  # B, 3, H, W
...
class DiscMixLogistic:
    def __init__(self, param, num_mix=10, num_bits=8):
        ...
        l = param[:, num_mix:, :, :].view(B, 3, 3 * num_mix, H,
                                          W)  # B, 3, 3 * M, H, W
        self.means = l[:, :, :num_mix, :, :]  # B, 3, M, H, W
        self.log_scales = torch.clamp(l[:, :, num_mix:2 * num_mix, :, :],
                                      min=-7.0)  # B, 3, M, H, W
        self.coeffs = torch.tanh(l[:, :, 2 * num_mix:3 *
                                   num_mix, :, :])  # B, 3, M, H, W
    ...
    def log_prob(self, samples):
        ...
        samples = samples.expand(-1, -1, -1, -1,
                                 self.num_mix).permute(0, 1, 4, 2,
                                                       3)  # B, 3, M, H, W
        ...
        log_probs = torch.where(
            samples < -0.999, log_cdf_plus,
            torch.where(samples > 0.99, log_one_minus_cdf_min,
                        log_prob_mid_safe))  # B, 3, M, H, W

        log_probs = torch.sum(log_probs, 1) + F.log_softmax(
            self.logit_probs, dim=1)  # B, M, H, W
